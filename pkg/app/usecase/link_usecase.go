...
import (
	...
	"github.com/satyamvatstyagi/LinkManagementService/pkg/common/env"
	"github.com/satyamvatstyagi/LinkManagementService/pkg/common/jwt"
	"github.com/satyamvatstyagi/LinkManagementService/pkg/common/restclient"
	"github.com/satyamvatstyagi/LinkManagementService/pkg/common/utils"
	"golang.org/x/crypto/bcrypt"
)

type userUsecase struct {
	userRepository models.LinkRepository
	httpClient     restclient.HTTPClient
}

func NewUserUsecase(userRepository models.LinkRepository, hc restclient.HTTPClient) domain.LinkUsecase {
	return &userUsecase{
...
func (u *userUsecase) RegisterUser(ctx context.Context, registerUserRequest *domain.RegisterUserRequest) (*domain.RegisterUserResponse, error) {
	...
	if err != nil {
		log.Println("[LinkUsecase][RegisterUser] Error in hashing the password: ", err)
		return nil, err
	}
	...
	if err != nil {
		log.Println("[LinkUsecase][RegisterUser] Error in RegisterUser: ", err)
		return nil, err
	}
...
func (u *userUsecase) LoginUser(ctx context.Context, loginUserRequest *domain.LoginUserRequest) (*domain.LoginUserResponse, error) {
	...
	if err != nil {
		log.Println("[LinkUsecase][LoginUser] Error in GetUserByUserName: ", err)
		return nil, err
	}

	// Compare the password
	if err := bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(loginUserRequest.Password)); err != nil {
		log.Println("[LinkUsecase][LoginUser] Error in CompareHashAndPassword: ", err)
		return nil, err
	}
	...
	if err != nil {
		log.Println("[LinkUsecase][LoginUser] Error in GenerateToken : ", err)
		return nil, err
	}
...
func (u *userUsecase) GetUserByUserName(ctx context.Context, getUserByUserNameRequest *domain.GetUserByUserNameRequest) (*domain.GetUserByUserNameResponse, error) {
	...
	if err != nil {
		log.Println("[LinkUsecase][GetUserByUserName] Error in GetUserByUserName: ", err)
		return nil, err
	}
...
func (u *userUsecase) SendRequestToServer(ctx context.Context, url string, requestJson []byte) (response []byte, err error) {
	...
	if err != nil {
		log.Println("[LinkUsecase][SendRequestToServer] Error in creating request: ", err)
		return nil, err
	}
	...
	if err != nil {
		log.Println("[LinkUsecase][SendRequestToServer] Error in sending request: ", err)
		return nil, err
	}

	// Check if the status code is 200
	if res.StatusCode != http.StatusOK {
		log.Println("[LinkUsecase][SendRequestToServer] Recieved ", res.StatusCode, " code from client")
	...
	if err != nil {
		log.Println("[LinkUsecase][SendRequestToServer] Error in reading response: ", err)
		return nil, err
	}
...
func (u *userUsecase) GetOrderByOrderUserName(ctx context.Context, getOrderByOrderUserNameRequest *domain.GetOrderByOrderUserNameRequest) (*domain.GetOrderByOrderUserNameResponse, error) {
	...
	if err != nil {
		log.Println("[LinkUsecase][GetOrderByOrderUserName] Error in GetUserByUserName: ", err)
		return nil, err
	}
	...
	if err != nil {
		log.Println("[LinkUsecase][GetOrderByOrderUserName] Error in unmarshalling response: ", err)
		return nil, err
	}
...
func (u *userUsecase) GetLinks(ctx context.Context) ([]string, error) {
	links, err := u.userRepository.GetLinks(ctx)
	if err != nil {
		log.Println("[LinkUsecase][GetLinks] Error in GetLinks: ", err)
		return nil, err
	}
